...
class _TabViewState extends State<CustomTabView> {
  Timer? closeTimer;
  ...
  Widget build(BuildContext context) {
    ...
    Widget tabBar = ScrollConfiguration(
        ...
        child: Row(children: [
          ...
          Expanded(
            child: LayoutBuilder(builder: (context, consts) {
              ...
              assert(
                width.isFinite,
                'You can only create a CustomTabView in a box with defined width',
              );
    ...
    if (widget.shortcutsEnabled) {
      ...
      int tabsShortcutCounter = widget.currentIndex;
      return FocusScope(
        autofocus: true,
        child: CallbackShortcuts(
          bindings: {
            ...
            SingleActivator(
              ...
              if (tabsShortcutCounter == widget.tabs.length - 1) {
                tabsShortcutCounter = 0;
                widget.onChanged?.call(tabsShortcutCounter);
              } else if (tabsShortcutCounter < widget.tabs.length) {
                widget.onChanged?.call(++tabsShortcutCounter);
              }
            },
          },
...
/// Represents a single tab within a [CustomTabView].
class CustomTab {
  final _tabKey = GlobalKey<__TabState>(debugLabel: 'CustomTab key');
      ...
      return HoverButton(
        ...
        builder: (context, states) {
          ...
          final backgroundColor = ButtonState.resolveWith<Color>((states) {
            ...
            final unselectedBaseColor = context.colorScheme.surfaceVariant;
            if (states.isPressing) {
              return widget.selected
              ...
              return widget.selected
                  ? selectedBaseColor.withOpacity(.4)
                  : unselectedBaseColor.withOpacity(.3);
            } else if (states.isDisabled) {
              return res.subtleFillColorDisabled;
            } else {
              return widget.selected
                  ? selectedBaseColor
                  : unselectedBaseColor.withOpacity(.1);
            }
          }).resolve(states);
